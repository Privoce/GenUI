use crate::builtin::prop::{Prop, VecValue, I32};
use crate::{
    builtin::{prop::err_from_to, widget::ViewProps},
...
impl TryFrom<(PropKey, Value)> for Props {
    ...
    fn try_from(value: (PropKey, Value)) -> Result<Self, Self::Error> {
        ...
        match name.as_str() {
            "selected" => Ok(Props::Selected((&value.1).try_into()?)),
            _ => {
                // handle deref widget
                if let Ok(p) = ViewProps::try_from(value) {
                    ...
                    return Err(err_from_to(
                        "GenUI Props",
                        &format!("Makepad GRadioGroup Prop, Invalid Prop: {}", name),
                    )
                    .into());
                }
            }
