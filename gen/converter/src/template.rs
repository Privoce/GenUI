...
pub struct TemplateModel {
    ...
    pub root: bool,
    /// 组件的子组件
    pub children: Option<Vec<TemplateModel>>,
    /// 记录父组件的标识
...
/// - 设置children
fn convert_template(tag: &Tag, is_root: bool) -> Result<TemplateModel, Error> {
    ...
    if tag.has_children() {
        let children = tag
            ...
            .iter()
            .map(|child| {
                let mut c_model = TemplateModel::convert(child, false).unwrap();
