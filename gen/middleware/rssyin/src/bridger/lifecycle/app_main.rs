...
pub struct AppLifeCycle {
    ...
    // /// 表示app的网络回调 see [HttpLifeCycle]
    // pub http: HttpLifeCycle, // 废弃，在GenUI中AppMain是生成出来的，网络部分都是在组件内部构建的
    /// 表示app的信号回调 The application has received a signal.
    /// 使用`#[signal]`标记
    pub signal: Option<ItemFn>,
}

impl TryFrom<(&Ident, &ItemFn)> for AppLifeCycle {
    type Error = crate::error::Error;

    fn try_from(value: (&Ident, &ItemFn)) -> Result<Self, Self::Error> {
        let mut res = AppLifeCycle::default();
...
impl ToTokens for AppLifeCycle {
