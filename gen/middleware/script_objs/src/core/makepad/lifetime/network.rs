...
#[allow(dead_code)]
#[derive(Debug, Clone)]
pub enum NetwrokLifeTime {
    Http(HttpLifeTime),
    // WebSocket(WebSocketLifeTime), TODO!("makepad 未支持WebSocket")
}

impl TryFrom<(&str, &ItemFn)> for NetwrokLifeTime {
    type Error = crate::error::Error;

    fn try_from(value: (&str, &ItemFn)) -> Result<Self, Self::Error> {
        let mut http = HttpLifeTime::default();
        match value.0 {
            "http_response" => {
            ...
            _ => {
                return Err(AttrMacroError::LifeTimeConflict(value.0.to_string()).into());
            }
        }
        Ok(http.into())
    }
...
impl From<HttpLifeTime> for NetwrokLifeTime {
    fn from(http: HttpLifeTime) -> Self {
        NetwrokLifeTime::Http(http)
    }
}
/// # 表示Http协议网络的生命周期回调
#[derive(Debug, Clone, Default)]
pub struct HttpLifeTime {
    /// 表示响应的回调
    /// 使用`#[http_response]`标记
...
impl HttpLifeTime {
    pub fn push(&mut self, attr: &str, item: &ItemFn) -> SCResult<()> {
        match attr {
            ...
            _ => {
                return Err(AttrMacroError::LifeTimeConflict(attr.to_string()).into());
            }
        }
        Ok(())
    }
...
impl ToTokens for HttpLifeTime {
