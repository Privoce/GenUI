...
pub struct WidgetLifeTime {
    ...
    /// 表示网络请求的回调
    pub http: HttpLifeTime,
}

impl TryFrom<(&Ident, &ItemFn)> for WidgetLifeTime {
    type Error = crate::error::Error;

    fn try_from(value: (&Ident, &ItemFn)) -> Result<Self, Self::Error> {
        let mut res = WidgetLifeTime::default();
...
impl ToTokens for WidgetLifeTime {
