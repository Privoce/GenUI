...
use gen_components::components::{
    button::GButtonWidgetExt, label::GLabelWidgetExt, loading::GLoadingWidgetExt, view::GView,
};
use makepad_widgets::*;
...
impl Widget for LoadingPage {
    ...
    fn handle_event(&mut self, cx: &mut Cx, event: &Event, scope: &mut Scope) {
        ...
        if open_btn1.clicked(&actions).is_some() {
            ld1.open(cx);
            o_label.set_text_and_redraw(cx, "Opened Loading1");
        }
        if close_btn1.clicked(&actions).is_some() {
            ld1.close(cx);
            o_label.set_text_and_redraw(cx, "Closed Loading1");
        }

        if open_btn2.clicked(&actions).is_some() {
            ld2.open(cx);
            o_label.set_text_and_redraw(cx, "Opened Loading2");
        }
        if close_btn2.clicked(&actions).is_some() {
            ld2.close(cx);
            o_label.set_text_and_redraw(cx, "Closed Loading2");
        }

        if open_btn3.clicked(&actions).is_some() {
            ld3.open(cx);
            o_label.set_text_and_redraw(cx, "Opened Loading3");
        }
        if close_btn3.clicked(&actions).is_some() {
            ld3.close(cx);
            o_label.set_text_and_redraw(cx, "Closed Loading3");
        }
    }
}
